<?php

namespace Wikimedia\LangConv\Tests\Language;

use PHPUnit\Framework\TestCase;
use Wikimedia\LangConv\ReplacementMachine;

class ZhTest extends TestCase {

	const CODES = [ "zh-cn", "zh-sg", "zh-my", "zh-hans", "zh-tw", "zh-hk", "zh-mo", "zh-hant" ];

	/** @var ReplacementMachine */
	private $machine;

	protected function setUp() {
		$this->machine = new ReplacementMachine( "zh", self::CODES );
	}

	protected function tearDown() {
		$this->machine = null;
	}

	/**
	 * @covers \Wikimedia\LangConv\FST
	 */
	public function testConvertZhHantToZhHans() {
		$input = "㑯";
		$output = [
			"zh"      => "㑯",
			"zh-hans" => "㑔",
			"zh-hant" => "㑯",
			"zh-cn"   => "㑔",
			"zh-hk"   => "㑯",
			"zh-mo"   => "㑯",
			"zh-my"   => "㑔",
			"zh-sg"   => "㑔",
			"zh-tw"   => "㑯",
		];
		foreach ( self::CODES as $variantCode ) {
			$expected = $output[$variantCode];
			$result = $this->convert( $input, $variantCode );
			$this->assertEquals( $expected, $result );
		}
	}

	/**
	 * @covers \Wikimedia\LangConv\FST
	 */
	public function testConvertZhHansToZhHant() {
		$input = "㐷";
		$output = [
			"zh"      => "㐷",
			"zh-hans" => "㐷",
			"zh-hant" => "傌",
			"zh-cn"   => "㐷",
			"zh-hk"   => "傌",
			"zh-mo"   => "傌",
			"zh-my"   => "㐷",
			"zh-sg"   => "㐷",
			"zh-tw"   => "傌",
		];
		foreach ( self::CODES as $variantCode ) {
			$expected = $output[$variantCode];
			$result = $this->convert( $input, $variantCode );
			$this->assertEquals( $expected, $result );
		}
	}

	/**
	 * @covers \Wikimedia\LangConv\FST
	 */
	public function testConvertZhCn() {
		$input = "仲介";
		$output = [
			"zh"      => "仲介",
			"zh-hans" => "仲介",
			"zh-hant" => "仲介",
			"zh-cn"   => "中介",
			"zh-hk"   => "仲介",
			"zh-mo"   => "仲介",
			"zh-my"   => "中介",
			"zh-sg"   => "中介",
			"zh-tw"   => "仲介",
		];
		foreach ( self::CODES as $variantCode ) {
			$expected = $output[$variantCode];
			$result = $this->convert( $input, $variantCode );
			$this->assertEquals( $expected, $result );
		}
	}

	/**
	 * @covers \Wikimedia\LangConv\FST
	 */
	public function testConvertZhHk() {
		$input = "中文里";
		$output = [
			"zh"      => "中文里",
			"zh-hans" => "中文里",
			"zh-hant" => "中文裡",
			"zh-cn"   => "中文里",
			"zh-hk"   => "中文裏",
			"zh-mo"   => "中文裏",
			"zh-my"   => "中文里",
			"zh-sg"   => "中文里",
			"zh-tw"   => "中文裡",
		];
		foreach ( self::CODES as $variantCode ) {
			$expected = $output[$variantCode];
			$result = $this->convert( $input, $variantCode );
			$this->assertEquals( $expected, $result );
		}
	}

	/**
	 * @covers \Wikimedia\LangConv\FST
	 */
	public function testZhTw() {
		$input = "甲肝";
		$output = [
			"zh"      => "甲肝",
			"zh-hans" => "甲肝",
			"zh-hant" => "甲肝",
			"zh-cn"   => "甲肝",
			"zh-hk"   => "甲肝",
			"zh-mo"   => "甲肝",
			"zh-my"   => "甲肝",
			"zh-sg"   => "甲肝",
			"zh-tw"   => "A肝",
		];
		foreach ( self::CODES as $variantCode ) {
			$expected = $output[$variantCode];
			$result = $this->convert( $input, $variantCode );
			$this->assertEquals( $expected, $result );
		}
	}

	/**
	 * @covers \Wikimedia\LangConv\FST
	 */
	public function testZhTwOverridesZhHant() {
		$input = "账";
		$output = [
			"zh"      => "账",
			"zh-hans" => "账",
			"zh-hant" => "賬",
			"zh-cn"   => "账",
			"zh-hk"   => "賬",
			"zh-mo"   => "賬",
			"zh-my"   => "账",
			"zh-sg"   => "账",
			"zh-tw"   => "帳",
		];
		foreach ( self::CODES as $variantCode ) {
			$expected = $output[$variantCode];
			$result = $this->convert( $input, $variantCode );
			$this->assertEquals( $expected, $result );
		}
	}

	/**
	 * @covers \Wikimedia\LangConv\FST
	 */
	public function testZhHkOverridesZhHant() {
		$input = "一地里";
		$output = [
			"zh"      => "一地里",
			"zh-hans" => "一地里",
			"zh-hant" => "一地裡",
			"zh-cn"   => "一地里",
			"zh-hk"   => "一地裏",
			"zh-mo"   => "一地裏",
			"zh-my"   => "一地里",
			"zh-sg"   => "一地里",
			"zh-tw"   => "一地裡",
		];
		foreach ( self::CODES as $variantCode ) {
			$expected = $output[$variantCode];
			$result = $this->convert( $input, $variantCode );
			$this->assertEquals( $expected, $result );
		}
	}

	private function convert( $input, $variantCode ) {
		return $this->machine->convert( $input, $variantCode, $this->getInvertCode( $variantCode ) );
	}

	private function getInvertCode( $variantCode ) {
		return (bool)preg_match( '/^zh-(cn|sg|my|hans)$/', $variantCode ) ? "zh-hant" : "zh-hans";
	}

}
