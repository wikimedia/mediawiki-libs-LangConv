...
class ConversionTraverser extends DOMTraverser {
	...
	aHandler(node, env, atTopLevel, tplInfo) {
		...
		if (rel === 'mw:WikiLink') {
			...
			// DU.usesURLLinkSyntax uses data-parsoid, but syntactic free
			// links should also have class="external free"
			if (DU.usesURLLinkSyntax(node) || node.classList.contains('free')) {
			...
			if (node.firstChild && DU.hasTypeOf(node.firstChild, 'mw:LanguageVariant')) {
				return true;
			}
